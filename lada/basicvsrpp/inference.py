...
def inference(model, video: list, device, max_frames=-1):
    ...
    # if torch.backends.mps.is_available() and str(device) == "mps":
    #     print(f"[DEBUG] inference: Using MPS device")
    #     print(f"[DEBUG] inference: Input frame count: {input_frame_count}")
    #     print(f"[DEBUG] inference: Input frame shape: {input_frame_shape}")
    ...
    with torch.no_grad():
        ...
        # Debug logging for tensor properties (commented out for production)
        # if torch.backends.mps.is_available() and str(device) == "mps":
        #     print(f"[DEBUG] inference: Input tensor shape: {input.shape}")
        #     print(f"[DEBUG] inference: Input tensor dtype: {input.dtype}")
        #     print(f"[DEBUG] inference: Input tensor device: {input.device}")

        if max_frames > 0:
            for i in range(0, input.shape[1], max_frames):
                batch_input = input[:, i : i + max_frames].to(device)
                # Debug logging for batch processing (commented out for production)
                # if torch.backends.mps.is_available() and str(device) == "mps":
                #     print(
                #         f"[DEBUG] inference: Processing batch {i//max_frames}, shape: {batch_input.shape}"
                #     )
                output = model(inputs=batch_input)
                result.append(output)
            result = torch.cat(result, dim=1)
        else:
            input = input.to(device)
            # Debug logging for full processing (commented out for production)
            # if torch.backends.mps.is_available() and str(device) == "mps":
            #     print(
            #         f"[DEBUG] inference: Processing all frames at once, shape: {input.shape}"
            #     )
